...
for i in "${!procs[@]}"; do
    ...
    if (( mb >= 500 )); then mb_color="#f87171"
    elif (( mb >= 150 )); then mb_color="#fde047"
    else mb_color="#00ffb3"; fi
...
# Tooltip — single clean line for RAM, ZRAM, and SWAP + processes table
tooltip="<span color='#eba0ac'>RAM:</span> <span color='${ram_color}'>${used_gb}/${total_gb} GiB</span> <span color='#6c7086'>(${percentage}%)</span>, <span color='#eba0ac'>ZRAM:</span> <span color='#89b4fa'>${zr_used}/${zr_size}G</span>, <span color='#eba0ac'>SWAP:</span> <span color='#94e2d5'>${sw_used}/${sw_size}G</span>\n"
tooltip+=" \n"
tooltip+=" ${col_header}\n ${col_divider}\n${rows}"
